...
import { type PathOrBackup, get } from './get'

type SelectorWithSuggestions<State, Path extends string, BackupValue = null> = <
  P extends string = Path,
>(
...
type StrictSelector<
  ...
  Path extends string = PathsOf<State>,
  BackupValue = null,
> = <P extends string = Path>(
  path: P,
  backupValue?: BackupValue
) => (s: State) => PathOrBackup<State, P, BackupValue>
type LooseSelector<State extends _Object, Path extends string = string, BackupValue = null> = <
  P extends string = Path,
>(
  path: P,
  backupValue?: BackupValue
...
type SelectorWithKnownPath<
  State extends _Object,
  Path extends PathsOf<State>,
  BackupValue = null,
> = (path: Path, backupValue?: BackupValue) => (s: State) => PathOrBackup<State, Path, BackupValue>
