...
function createSelectors<AppState extends _Object, Prefix extends string>(
  initialState: AppState,
  paths?: Array<PathsOf<AppState>>,
  prefix?: Prefix
): SelectorsObject<AppState>
function createSelectors(initialState, paths, prefix) {
  let selectorPaths: string[] = []
  const acc = {}

  selectorPaths = paths == null ? getObjectPaths(initialState) : [paths].flat()
